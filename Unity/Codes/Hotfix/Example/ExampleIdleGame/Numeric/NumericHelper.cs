...
namespace ET
{
    public static class NumericHelper
    {
        ...
        public static async ETTask<int> RequestAddAttributePoint(Scene zoneScene, int numericType)
        {
            ...
            try
            {
                m2CAddAttributePoint = (M2C_AddAttributePoint)await zoneScene.GetComponent<SessionComponent>().Session.Call(
                    new C2M_AddAttributePoint() { NumericType = numericType });
            }
            catch (Exception e)
            {
                Log.Error(e.ToString());
