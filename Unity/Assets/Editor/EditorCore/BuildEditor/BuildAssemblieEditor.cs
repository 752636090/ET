...
namespace ET
{
    public static class BuildAssemblieEditor
    {
        ...
        private const string EditorCodeDir = "Assets/Editor/Dll/";
        ...
        [MenuItem("Tools/Build/BuildCodeEditor #F5")]
        public static void BuildCodeEditor()
        {
            while (EditorApplication.isCompiling)
            {
                Debug.Log("Compiling wait1");
            ...
            Thread.Sleep(1000);
            BuildAssemblieEditor.BuildMuteAssembly("Editor",new[]
            {
                "Codes/Model/",
                "Codes/ModelView/",
                "Codes/Hotfix/",
            ...
            //new[]
            //{
            //    $"{Define.BuildOutputDir}/Code.dll",
            //}
            Array.Empty<string>()
            ...
            File.Copy(Path.Combine(Define.BuildOutputDir, "Editor.dll"), Path.Combine(EditorCodeDir, "Editor.dll"), true);
            File.Copy(Path.Combine(Define.BuildOutputDir, "Editor.pdb"), Path.Combine(EditorCodeDir, "Editor.pdb"), true);
        ...
        private static void BuildMuteAssembly(string assemblyName, string[] CodeDirectorys, string[] additionalReferences, CodeOptimization codeOptimization)
        {
            ...
            assemblyBuilder.flags = assemblyName != "Editor" ? AssemblyBuilderFlags.None : AssemblyBuilderFlags.EditorAssembly;
            ...
            assemblyBuilder.excludeReferences = new string[] { "Assets/Editor/Dll/Editor.dll" };
